...
function calculateDayInYear(date) {
  var splitDate = date.split('/'); //converts dates to array
  var year = Number(splitDate[0]); //takes year from splitDate array
  var month = Number(splitDate[1]); //takes month from splitDate array
  var day = Number(splitDate[2]); //takes day from splitDate array
  var febDays = daysInFeb(year); //daysinFeb error (28 days except leapyear)
...
} //need conditional statement for leapyear. 
